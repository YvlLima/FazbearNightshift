...
| `/status` | `[utilizador]` *(opcional)* | Consulta o HP, progresso do Ennard/Scott e efeitos ativos do jogador (ephemeral). |
| `/colecao` | `[utilizador]` *(opcional)* | Consulta a coleção de animatronics encontrados (ephemeral) e progresso de desbloqueio do Ennard/Scott. |
| `/perfil` | `[utilizador]` *(opcional)* | Consulta as estatísticas detalhadas, vitórias, KOs e animatronic favorito (ephemeral). |
| `/animatronic` | `nome` *(obrigatório)* | Consulta a ficha técnica, dano e poder especial de qualquer animatronic com autocomplete (ephemeral). |
| `/leaderboard` | *Nenhum* | Exibe o ranking Top 10 dos jogadores com mais eliminações (KOs). |
| `/servidor-ranking` | *Nenhum* | Exibe o ranking dos jogadores do servidor atual. |
| `/historico` | `[utilizador]` *(opcional)* | Consulta o histórico dos últimos duelos PvP realizados. |
| `/ajuda` | *Nenhum* | Apresenta o guia completo de mecânicas, segredos e comandos do bot. |
